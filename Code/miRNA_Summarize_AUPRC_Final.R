...
for(BipartiteNet in BipartiteNets){
  cat("Calculating AUPRC for ", BipartiteNet, "\n")
  ...
  for(Method in Methods){
    ...
    # Read the data (assuming the file contains 'Scores' and 'Labels' columns)
    df.ScoreLabel = read.csv(Result_by_Score_n_LabelFile)
    ...
    # aupravgbyAll = ratio*aupravgbyAll
    print(paste("AUPRC:", aupravgbyAll))
    
    
    if(Method=="MH"){
      Network = rep(paste0("MHMDA-MH (AUPRC = ",aupravgbyAll,")"),length(Recall))
    }else if(Method=="H1"){
      Network = rep(paste0("RWRHMDA (MonoNet_miRWalk) (AUPRC = ",aupravgbyAll,")"),length(Recall))
    }else if(Method=="H2"){
      Network = rep(paste0("RWRHMDA (MonoNet_TargetScan) (AUPRC = ",aupravgbyAll,")"),length(Recall))
    }else if(Method=="H3"){
      Network = rep(paste0("RWRHMDA (MonoNet_Integrated) (AUPRC = ",aupravgbyAll,")"),length(Recall))
    }else if(Method=="M12"){
      Network = rep(paste0("MHMDA-M (AUPRC = ",aupravgbyAll,")"),length(Recall))
    }else if(Method=="M1"){
      Network = rep(paste0("RWRMDA (MonoNet_miRWalk) (AUPRC = ",aupravgbyAll,")"),length(Recall))
    }else if(Method=="M2"){
      Network = rep(paste0("RWRMDA (MonoNet_TargetScan) (AUPRC = ",aupravgbyAll,")"),length(Recall))
    }else{
      Network = rep(paste0("RWRMDA (MonoNet_Integrated) (AUPRC = ",aupravgbyAll,")"),length(Recall))
    }
...
for(BipartiteNet in keys(h.All)){
  ...
  df.O = df.O[(df.O$Precision!=0) & (df.O$Recall!=0),]
  ...
  p <- ggplot(df.D, aes(x=Recall, y=Precision, shape=Network, color = Network)) +
    # geom_point() +
    geom_line(size=1) +
...
library('cowplot')
plot_grid(l.plot[["HMDD"]], l.plot[["miR2Disease"]], labels=c("(c)", "(d)"), ncol = 2, nrow = 1)

ggsave(paste0(FigureFile,"_AUPR_Manipulated.pdf"), width = 11, height = 5.5)
