...
for(BipartiteNet in BipartiteNets){
  ...
  for(Method in Methods){
    ...
    if(Method=="MH"){
      ...
      Network = rep(paste0("DiSimNet-MonoNet_Integrated (AUC = ",aucavgbyAll,")"),length(FPR))  
    }else if(Method=="M12"){
      ...
      Network = rep(paste0("MonoNet_Integrated (AUC = ",aucavgbyAll,")"),length(FPR))  
    }
...
for(BipartiteNet in keys(h.All)){
  ...
  p <- ggplot(df.D, aes(x=FPR, y=TPR, shape=Network, color = Network)) +
    ...
    xlab("\nTrue Positive Rate") + #Set x label
...
ggsave(paste0(FigureFile,"_New.pdf"), width = 11, height = 5.5)
